...
if [ -f ".lab-entry-draft.json" ]; then
    echo "📊 Found active lab entry..."
    BLOCK_COUNT=$(jq '.blocks | length' .lab-entry-draft.json)
    FIRST_BLOCK=$(jq -r '.blocks[0].timestamp' .lab-entry-draft.json)
    LAST_BLOCK=$(jq -r '.blocks[-1].timestamp' .lab-entry-draft.json)
    ...
    COMMIT_COUNT=$(jq '[.blocks[].commits | length] | add' .lab-entry-draft.json)
    echo "  - Commits tracked: $COMMIT_COUNT"
    
    # Get commit SHAs
    COMMITS=$(jq -r '.blocks[].commits[].sha' .lab-entry-draft.json | tr '\n' ', ')
    echo "  - Commits: ${COMMITS%,}"
else
    echo "⚠️  No active lab entry found"
    echo "  Run: .vibe/skills/lab_entry/lab_entry.sh start"
fi
...
if [ -f ".lab-entry-draft.json" ]; then
    ...
    for ((i=0; i<BLOCK_COUNT; i++)); do
        TIMESTAMP=$(jq -r ".blocks[$i].timestamp" .lab-entry-draft.json)
        NOTE=$(jq -r ".blocks[$i].note" .lab-entry-draft.json)
        COMMIT_COUNT=$(jq ".blocks[$i].commits | length" .lab-entry-draft.json)
...
echo "  - Time: $TIME"
echo "  - User: $USER"
echo "  - Lab entry: $([ -f ".lab-entry-draft.json" ] && echo "Active" || echo "None")"
echo ""
echo "💡 Next steps:"

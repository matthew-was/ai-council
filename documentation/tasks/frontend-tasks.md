...
### Task F-029: GitHub Actions — frontend CI workflow

**Description**: Create the GitHub Actions workflow that runs the frontend test suite and build check. Tests run against MSW handlers (no backend required). The build downloads the `openapi-spec` artifact from the latest successful backend CI run to generate real types; it falls back to a minimal committed stub only when no artifact is available (e.g. on a frontend-only branch before any backend CI has run).

Produce:
- `.github/workflows/frontend-ci.yml` — workflow triggered on `push` and `pull_request` for paths matching `apps/frontend/**` and `.github/workflows/frontend-ci.yml`. One job:
  - **test** (runs on `ubuntu-latest`):
    - `actions/checkout@v4`
    - `actions/setup-node@v4` with `node-version: '20'`; npm cache keyed on `apps/frontend/package-lock.json`
    - Install: `npm ci` from `apps/frontend/`
    - **Download OpenAPI spec**: use `dawidd6/action-download-artifact@v6` to attempt downloading the `openapi-spec` artifact from the latest successful run of the `backend-ci` workflow on the same branch (falling back to `main` if no match). Set `if-no-artifact-found: warn` so the step does not fail when no artifact exists yet.
    - **Generate types**: if `openapi.json` was downloaded, run `npx openapi-typescript openapi.json --output apps/frontend/src/api/schema.ts`; otherwise copy `apps/frontend/src/api/schema.stub.ts` to `apps/frontend/src/api/schema.ts`.
    - Test: `npm run test` (Vitest with MSW — no backend required; runs against the generated or stub types)
    - Build: `npm run build` with `SKIP_TYPE_GENERATION=true` to prevent `generate-types.sh` from attempting a live backend fetch (types are already in place from the step above)

- `apps/frontend/src/api/schema.stub.ts` — committed minimal stub containing empty exported type aliases for every type imported by `src/types/app.ts`. Used only when the backend has not yet published a CI artifact. Contains a comment explaining its purpose. Listed in `.gitignore` for `schema.ts` (the generated file) but the stub itself is committed.

- Update `apps/frontend/scripts/generate-types.sh` to exit 0 immediately if `SKIP_TYPE_GENERATION=true` is set (types are already generated by the CI step above).

- Update `apps/frontend/vite.config.ts` pre-build plugin to respect `SKIP_TYPE_GENERATION`.

Add a `frontend-ci` status badge to `README.md`.

**Depends on**: F-005

**Complexity**: S

**Acceptance condition**: A push triggers the workflow. When a backend CI artifact exists: the `openapi-spec` artifact is downloaded, `schema.ts` is generated from it, tests pass, and `vite build` completes without TypeScript errors. When no artifact exists: the stub is copied to `schema.ts`, tests pass, and `vite build` completes. Both paths confirmed by viewing workflow run logs in the Actions tab.
